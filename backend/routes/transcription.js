...
router.post(
  ...
  async (req, res, next) => {
    try {
      ...
      if (!apiKey) {
        return res.status(500).json({ error: true, message: 'BACKEND_API_KEY غير مضبوط' });
      }
      // تحقق من الملف
      const file = req.file;
      if (!file) {
        return res.status(400).json({ error: true, message: 'ملف الصوت مفقود' });
      }
      const { application_id, question_id } = req.body || {};
      if (!application_id || !question_id) {
        return res.status(400).json({ error: true, message: 'application_id و question_id مطلوبان' });
      }
      ...
      if (!resp?.success) {
        return res.status(500).json({ error: true, message: resp?.message || 'فشل المعالجة' });
      }
