...
import { extractAgeAndExperience } from '@/utils/candidateParsing'
...
export async function getCandidateForExport(candidateId: string) {
  ...
  // Extract age and experience if missing
  let age = application.candidate_age;
  let experience = application.experience || 0;

  if ((age === undefined || experience === 0) && application.answers && Array.isArray(application.answers)) {
    application.answers.forEach((answer: any) => {
      if (answer.value) {
        const valueStr = String(answer.value).toLowerCase();
  ...
  // Assign back extracted values
  if (age !== undefined) application.candidate_age = age;
  if (experience !== 0) application.experience = experience;
